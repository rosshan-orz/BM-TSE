...
# 由于其他模块没有变化，这里只提供精简后的 NeuroSecure 类
# Sandglasset, TasNetEncoder, etc. 保持原样，只是初始化参数变了

class Separator(nn.Module):
    """
    一个精简后的多模态分离器，适用于显存受限的环境。
    
    Args:
        encoder_window_length (int): 编码器窗口长度。保持不变，因为这与输入数据格式有关。
        encoder_out_channels (int): 编码器输出通道数，从 256 降至 128。
        encoder_out_features (int): 编码器输出特征维度，从 256 降至 128。
        K (int): 分块长度，保持不变。
        R (int): Sandglasset 块的数量，从 6 降至 4。
        H (int): Sandglasset 块中 RNN 的隐藏层大小，从 128 降至 64。
        source_num (int): 要分离的声源数量，保持不变。
        n_classes (int): EEG 编码器所需的类别数，保持不变。
    """
    def __init__(self, 
                 encoder_window_length=4, 
                 encoder_out_channels=128,  # 精简：从 256 降至 128
                 encoder_out_features=128,  # 精简：从 256 降至 128
                 K=256, 
                 R=6,                       # 精简：从 6 降至 4
                 H=128,                      # 精简：从 128 降至 64
                 source_num=1,
                 ):
    ...
    def forward(self, audio_waveform, eeg_features, brainmap):
        # forward 方法保持不变
        # audio_waveform [B, T] or [B, 1, T]
        if audio_waveform.dim() == 3:
            audio_waveform = audio_waveform.squeeze(1)
        audio_features, x_wave, encoder_padding = self.tasnet_encoder(audio_waveform)
        # audio_features [B, N, T]
        # B: batch size
        # N: number of filters
        # T: number of frames
