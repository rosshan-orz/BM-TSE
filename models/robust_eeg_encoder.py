...
# Long Short Temporal Convolution
...
class AN_layer(nn.Module):
    def __init__(self, channel, sa_groups=4, size=None):
        super(AN_layer, self).__init__()
...
class RobustEEGEncoder(nn.Module):
    def __init__(self, spa_dim=16, patch_size=16, time_sample_num=256, emb_size=128, 
        ...
        self.an_temporal = AN_layer(channel=self.D, sa_groups=4, size=(1, self.P))
        self.log_power_temporal = LogPowerLayer(dim=-1, length=2, step=2)
        self.an_spatial = AN_layer(channel=self.D, sa_groups=4, size=(1, self.C))
        self.log_power_spatial = LogPowerLayer(dim=-1, length=2, step=2)
    ...
    def forward(self, x):
        ...
        an_temp_out = self.an_temporal(x_temporal_4d).squeeze(2).permute(0, 2, 1)
        ...
        an_spat_out = self.an_spatial(x_spatial_4d).squeeze(2).permute(0, 2, 1)
        log_spat_out = self.log_power_spatial(x_spatial_4d).squeeze(2).permute(0, 2, 1)

        an_temp_resized = self._resize_sequence(an_temp_out, self.temporal_resizer)
        ...
        x_fused_temporal = torch.cat([an_temp_resized, log_temp_resized], dim=1)
        x_fused_spatial = torch.cat([an_spat_out, log_spat_resized], dim=1)
